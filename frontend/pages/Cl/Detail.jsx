...
function MetaRail({ cl }) {
  return (
    <div className="dmeta">
      <div className="dmeta-head">Properti</div>
      <div className="dmeta-row"><span className="dmeta-k">Hotel</span><span className="dmeta-v">{cl.hotel_name || "—"}</span></div>
      <div className="dmeta-row"><span className="dmeta-k">Agen Travel</span><span className="dmeta-v">
        {cl.client ? <a href={`/clients/${cl.client.pk}/`} style={{ color: "var(--accent-2)", textDecoration: "none" }}>{cl.client.name}</a> : (cl.guest_name || "—")}
      </span></div>
      <div className="dmeta-row"><span className="dmeta-k">Telepon</span><span className="dmeta-v mono">{cl.guest_phone || "—"}</span></div>
      ...
      <div className="dmeta-row"><span className="dmeta-k">Malam</span><span className="dmeta-v">{cl.num_nights}</span></div>
      <div className="dmeta-row"><span className="dmeta-k">Tamu</span><span className="dmeta-v">{cl.num_guests} orang</span></div>
  ...
  return (
    <div className="card">
      <div className="card-header">
        <span className="card-title">Penalti Pembatalan</span>
        {penalty ? (
          ...
          <a href={`/cl/${cl.pk}/penalty/new/`} className="btn btn-primary btn-sm">+ Buat Dokumen Penalti</a>
        )}
      </div>
      <div className="card-body">
        {penalty ? (
          <div className="grid-2">
            <div className="field"><div className="field-label">No. Penalti</div><div className="field-value"><a href={`/penalty/${penalty.pk}/`} style={{ color: "var(--accent)", fontFamily: "monospace", fontWeight: 600 }}>{penalty.penalty_number}</a></div></div>
            <div className="field"><div className="field-label">Tanggal Batal</div><div className="field-value">{penalty.cancellation_date}</div></div>
            <div className="field"><div className="field-label">Jumlah Penalti</div><div className="field-value" style={{ fontWeight: 700 }}>{fmt(penalty.penalty_amount)} {penalty.penalty_currency}</div></div>
            <div className="field"><div className="field-label">Status Bayar</div><div className="field-value">{penalty.is_paid ? <span className="badge badge-green">Lunas</span> : <span className="badge badge-red">Belum Dibayar</span>}</div></div>
          </div>
        ) : (
          <p style={{ color: "var(--text-2)", fontSize: 13 }}>Belum ada dokumen penalti untuk CL ini.</p>
        )}
      </div>
    </div>
  );
...
function RoomsTable({ cl, rooms }) {
  return (
    <div className="card">
      <div className="card-header"><span className="card-title">Kamar</span></div>
      <div className="table-wrap">
        <table>
          <thead>
            <tr><th>Tipe</th><th className="col-m-hide">Meals</th><th className="col-num col-m-hide">Qty</th><th className="col-num col-m-hide">Harga/Malam</th><th className="col-num">Subtotal</th></tr>
          </thead>
          <tbody>
            {rooms.length ? rooms.map((room, i) => (
              <tr key={i}>
                <td className="col-m-primary">
                  {room.room_type}
                  <span className="m-sub">{room.quantity} × {fmt(room.price)} SAR/malam</span>
                </td>
              ...
              <tr><td colSpan={5} style={{ textAlign: "center", color: "var(--text-3)", padding: 20 }}>Tidak ada data kamar</td></tr>
            )}
          </tbody>
          <tfoot>
            <tr>
      ...
      <div className="dhero">
        ...
        <div className="dhero-side">
          <div className="dhero-badges">
            ...
            {cl.invoice && (
              <a href={`/invoice/${cl.invoice.pk}/`} className="badge badge-blue" style={{ textDecoration: "none" }} title="Ditagih dalam invoice ini">
                Ditagih → {cl.invoice.invoice_number}
              </a>
            )}
          </div>
