      ...
      {rooms.map((r, i) => {
        ...
        return (
          <div key={i} className="fg-2" style={{ gridTemplateColumns: "1.2fr 1fr .7fr 1fr auto", alignItems: "end", gap: 8, marginBottom: 8 }}>
            ...
            <div className="ff"><label>Meals</label><input type="text" value={r.meals} onChange={(e) => update(i, "meals", e.target.value)} placeholder="BB, HB…" /></div>
            <div className="ff"><label>Qty</label><input type="number" min="1" value={r.quantity} onChange={(e) => update(i, "quantity", e.target.value)} /></div>
            <div className="ff"><label>Price/night</label><input type="number" min="0" step="0.01" value={r.price} onChange={(e) => update(i, "price", e.target.value)} placeholder="0.00" /></div>
            <button type="button" className="btn btn-ghost btn-sm" onClick={() => remove(i)} title="Delete" style={{ marginBottom: 2 }}>×</button>
