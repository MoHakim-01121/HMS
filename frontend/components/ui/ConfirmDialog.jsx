...
export default function ConfirmDialog({
  ...
  useEffect(() => {
    if (!open) return;
    // Enter is handled natively by whichever button has focus (autoFocus below
    // puts it on Confirm) — no manual Enter branch here, or Tab-ing to Cancel
    // and pressing Enter would still fire onConfirm regardless of focus.
    const onKey = (e) => {
  ...
  return (
    ...
    >
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="confirm-dialog-title" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-xl)", width: "100%", maxWidth: 400, overflow: "hidden", boxShadow: "0 24px 48px rgba(0,0,0,.4)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "24px 24px 20px", borderBottom: "1px solid var(--border)" }}>
          ...
          <div>
            <div id="confirm-dialog-title" style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{title}</div>
