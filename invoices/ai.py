...
_MODEL_SMALL = "llama-3.2-1b-preview"  # summary & draft — hemat token
_MODEL_CHAT  = "llama-3.1-8b-instant"  # chat interaktif
_logger = logging.getLogger(__name__)


def _call_groq(prompt: str, model: str = _MODEL_SMALL) -> str | None:
...
def generate_invoice_summary(invoice) -> None:
    ...
    result = _call_groq(prompts.invoice_summary(invoice, total, paid, remaining, hotels))
    if result:
...
def generate_cl_summary(cl) -> None:
    ...
    result = _call_groq(prompts.cl_summary(cl, rooms_desc))
    if result:
...
def generate_services_summary(invoice) -> None:
    ...
    result = _call_groq(prompts.services_summary(invoice, services_desc, total, remaining))
    if result:
        invoice.ai_summary = result
        invoice.save(update_fields=["ai_summary"])
...
def generate_draft_message(invoice_type: str, invoice) -> str | None:
    ...
    if invoice_type == "invoice":
        ...
        return _call_groq(prompts.draft_invoice(invoice, hotels, total, paid, remaining, due_info))
    else:
        items = invoice.service_items.all()
        total = sum(int(i.qty * float(i.price)) for i in items)
        paid = sum(float(p.amount) for p in invoice.payments.all())
        remaining = total - paid
        services_desc = ", ".join(i.name for i in items[:4]) or "-"
        return _call_groq(prompts.draft_services(invoice, services_desc, total, paid, remaining))


def get_chat_reply(message: str, company: str | None = None) -> str | None:
    ...
    inv_qs = Invoice.objects.filter(invoice_type="hotel")
    svc_qs = Invoice.objects.filter(invoice_type="visa")
    cl_qs  = ConfirmationLetter.objects.all()
    if company:
        inv_qs = inv_qs.filter(company=company)
        svc_qs = svc_qs.filter(company=company)
        cl_qs  = cl_qs.filter(company=company)

    invoices = list(inv_qs.order_by("-created_at")[:50])
    services = list(svc_qs.order_by("-created_at")[:50])
    cls = list(cl_qs.order_by("-created_at")[:50])
    ...
    inv_lines = "\n".join(
        f"- INV#{i.invoice_number} | {i.customer_name} | {i.total_sar:,.0f} SAR | "
        f"Sisa: {i.remaining_sar:,.0f} SAR | {'Lunas' if i.remaining_sar == 0 else 'Belum lunas'} | "
        f"{i.issued_date or '-'}"
        for i in invoices[:20]
    )
    svc_lines = "\n".join(
        f"- SVC#{i.invoice_number} | {i.customer_name} | {i.currency} | {i.issued_date or '-'}"
        for i in services[:20]
    )
    cl_lines = "\n".join(
        f"- CL#{c.confirmation_number} | {c.guest_name} | {c.hotel_name} | "
    ...
    context = (
        f"Kamu adalah asisten HMS (Hotel Management System). Jawab pertanyaan berdasarkan data berikut.\n"
        f"Jawab dalam Bahasa Indonesia, singkat dan langsung ke poin. Hari ini: {today}.\n"
        f"Data yang ditampilkan hanya milik: {company_label}.\n\n"
        f"=== RINGKASAN ===\n"
        ...
        f"=== INVOICE HOTEL (20 terbaru) ===\n{inv_lines}\n\n"
        f"=== INVOICE SERVICES (20 terbaru) ===\n{svc_lines}\n\n"
        f"=== CONFIRMATION LETTER (20 terbaru) ===\n{cl_lines}\n\n"
        f"Pertanyaan: {message}"
    )
    return _call_groq(context, model=_MODEL_CHAT)
