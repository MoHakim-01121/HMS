...
def _logo_file_url(company):
    ...
    path = Path(__file__).resolve().parent.parent / "static" / "invoices" / "img" / filename
...
def _render_cl_pdf(cl):
    ...
    template = "invoices/cl/cl_pdf_ijabah.html" if cl.company == "ijabah" else "invoices/cl/cl_pdf_konoz.html"
...
def _render_invoice_pdf(invoice):
    ...
    template = "invoices/invoice/invoice_pdf_ijabah.html" if invoice.company == "ijabah" else "invoices/invoice/invoice_pdf.html"
    html = render_to_string(template, context)
    pdf = HTML(string=html, base_url=str(settings.BASE_DIR)).write_pdf()
...
def _render_services_pdf(invoice):
    ...
    html = render_to_string("invoices/services/invoice_pdf_visa.html", context)
    pdf = HTML(string=html, base_url=str(settings.BASE_DIR)).write_pdf()
